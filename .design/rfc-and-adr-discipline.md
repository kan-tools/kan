...
- REQ-1: `rfcs/0-rfc-and-adr-process.md` must define the distinct purposes,
...
- REQ-3: `rfcs/README.md`, `rfcs/0-rfc-and-adr-process.md`, and
...
- REQ-5: Every ADR currently headed `ADR-1` through `ADR-91` in
  `docs/DECISIONS.md` must appear exactly once as an unpadded file under
...
- [ ] AC-1: A validator reports RFC 0 and both templates contain every
      required section and only recognized statuses. (REQ-1, REQ-3, REQ-4)
- [ ] AC-2: RFC 0 states allocation-at-PR-open, permanent gaps, maintainer
      acceptance, and a minimum 72-hour review period. (REQ-2)
- [ ] AC-3: The ADR migration validator reports exactly 91 unique records,
      numbered 1 through 91 without leading zeroes, with matching titles and preserved source
      prose. (REQ-5, REQ-8)
...
- [ ] AC-6: `CONTRIBUTING.md` states the RFC/ADR boundary and duplication rule,
      and its RFC link resolves to RFC 0. (REQ-9)
- [ ] AC-7: RFC 0 identifies the kan URI scheme as the expected next RFC
...
RFC 0 is self-applying where possible. Because it is the act that creates
...
- RFCs govern forward-looking reviewed proposals; ADRs record decisions
  ...
  proposals.
- Each ADR has its own unpadded file and `adrs/README.md` is the index.
...
- Specifying the kan URI grammar, resolution algorithm, or equivalence rules;
  that is the intended RFC 1 after RFC 0 merges.
