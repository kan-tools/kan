...
(`src/store/log.rs:798`) and `Log::iter_all` (`:1058`) already have exactly
...
| **M1.5** | `GitTree` transport (ADR-43) + the schema-evolution contract it forced (ADR-44, `docs/SPEC.md` §7.1). Inserted ahead of `HostedRelay`: the second `Transport` implementation M0 waited for, at a fraction of the cost, and #7's E2EE does not gate it — a git remote already trusted with the source tree is a different threat model from an untrusted relay | **v0.6.0-beta.1** | **Shipped — publish half only.** `GitTree::subscribe` exists and nothing calls it; `Workspace` does not know `Transport`. A repo can publish claims; a clone cannot yet consume them |
| **M2** | Thread `Transport` through `Workspace` so a published tree is actually *read* — the wiring M0 deferred until a second implementation existed (REQ-3), which M1.5 now provides. This is what makes kan genuinely multi-actor rather than merely capable of it | v0.7.0-beta.1 | Not started; unblocked by M1.5 |
| M3 | Issue #30 `/design` + build — real per-agent identity | v0.8.0-beta.1 | Not started; parallel track, gates `HostedRelay` shipping, not starting. Issue #69 (keychain friction) should be resolved first — #30 multiplies the number of keys and therefore the number of approval prompts |
| M4 | `HostedRelay` `/design` pass + build, informed by M1's resolution. **Sharpened by `.design/sync-remote-and-publicness-ladder.md` (ADR-54):** its first deployment is the personal encrypted backup — HostedRelay at N=1 with the cross-actor fold off; the wire shape is atproto repo-sync (not git — the append-only MST makes it lighter); and its design pass must resolve the blind-backup-vs-AppView E2EE fork per-rung rather than assume one posture | v0.9.0-beta.1 | Not started; depends on M1 |
...
| M5 | `AtProto`/PDS/firehose transport, `docs/SPEC.md` §10.1's lexicon separation (`*.claim`/`*.relation.sameAs`/`*.trust.*`) | v1.x / v2 | Not started; depends on M3 proving the AppView/sync mechanics. Deliberately *not* a 1.0 blocker — `docs/SPEC.md` §10 frames `AtProto` as ecosystem expansion ("lexicons = evangelism"), `HostedRelay` as the core product ("the monetizable one") |
