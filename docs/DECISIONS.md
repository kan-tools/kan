...
doc:**
- `atproto-repo`'s `CarWriter` has no incremental-append mode — writing means
  serializing every reachable block fresh. `Log` keeps everything in one
  `MemoryStorage` for its lifetime and does a full CAR-file rewrite on every
  `append`, O(n) instead of O(1). Correctness-first over fast-but-unverified,
  consistent with the house rules — but worth watching as real usage
  accumulates (tracked in issue kan-tools/kan#8).
- No `HEAD` sidecar file needed anymore (unlike ADR-8's `atrium-repo`
  workaround) — `CarWriter`'s header `roots` are rewritten fresh every time,
  so `CarReader::root()` on open is simply correct.
...
- `tests/log_stress.rs` is a permanent regression guard: 60 sequential
  appends through the real `Log` API, checking every prior claim's
  reachability after every single append, plus a fresh-reopen check.
  Institutionalizes the exact check that caught ADR-11's bug so a similar
  regression (in `atproto-repo` or in kan's own usage of it) fails CI
  immediately rather than surfacing as silent data loss later.
