...
## Phase 3 — First build (spine only — the v1 fence)

*Dependency order. This is the "one human, one agent, one repo, no sync" target.*

- [ ] `Claim` struct + DAG-CBOR canonicalization + CID (content-excluding-sig) + signing
- [ ] Local append-only signed log = source of truth (atproto-style record collection on disk)
- [ ] Disposable SQLite index (pure projection; rebuildable)
- [ ] The **fold**:
  - [ ] identity fold (witness-retaining, NOT plain union-find; clique-cached) + `SoloTrust` + `PeerContested`
  - [ ] state fold (poset → maximal antichain → `Settled | Confirmed | Contested`)
  - [ ] identity-before-state, same enrichment; decategorify only at `render`
- [ ] Anchors (git: Workspace/Commit/Blob/FileAt/LineRangeAt) + admissibility invariant
- [ ] Computable relation providers: `GitAncestry` + `GitSameFile` (default-on, disableable)
- [ ] CLI (git-like verbs: `kan observe|plan|decide|resolve|same|show|issues|status|session|context`)
- [ ] MCP server: claim-append + **budgeted context assembly** (the actual product)
...
## Phase 4 — Tests (write alongside Phase 3, not after)

- [ ] Fixture: local-only trivial case (one log, latest-wins)
- [ ] Fixture: two-actor contested status (the poset/antichain case)
- [ ] Fixture: `SameAs` merge + retraction-split (witness retention + component re-derivation)
- [ ] Property: `fold` is deterministic in (claim set, enrichment)
- [ ] Guardrail test: identity component size > N flags instead of enumerating
