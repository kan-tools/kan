...
RFCs live at `rfcs/NNNN-slug.md`. A number is allocated when the proposal pull
request opens. It MUST NOT be reused, including when its proposal is withdrawn
or rejected; permanent gaps are valid historical evidence. RFC 0000 is the
self-applying process record and is accepted by the pull request that first
merges it.
...
ADRs live at `adrs/NNNN-slug.md` and are indexed by `adrs/README.md`. Numbers
are allocated monotonically when the ADR is merged. Existing numbers 0001
through 0091 retain their historical order.
...
splits each into a zero-padded file, preserves its title and original prose in
...
RFC and ADR numbers are canonical identities. Filenames add human-readable
slugs, but references SHOULD use the number and MAY omit the slug in prose.
Renaming a title or slug does not create a new record. Reusing a number does.
...
substrate semantics are deliberately outside RFC 0000.
