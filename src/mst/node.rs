...
pub struct MstNode {
    ...
    ///
    /// Serialized as an explicit `null` when absent, NOT omitted. The MST node
    /// schema has `l` present-but-nullable, and in DAG-CBOR an absent field and
    /// a null field are different bytes and therefore a different CID. Omitting
    /// it (`skip_serializing_if`, as upstream 0.14.5 does) makes every node
    /// disagree with a conformant implementation, including single-entry nodes.
    #[serde(rename = "l")]
    pub left: Option<Cid>,
...
impl MstNode {
    ...
    pub fn keys(&self) -> Result<Vec<String>, super::Error> {
    ...
    ///
    /// Returns error if key reconstruction fails.
    pub fn key_values(&self) -> Result<Vec<(String, Cid)>, super::Error> {
        let mut reconstructor = KeyReconstructor::new();
    ...
    ///
    /// Returns error if key reconstruction fails.
    pub fn find_entry(&self, key: &str) -> Result<Option<(usize, &TreeEntry)>, super::Error> {
        let mut reconstructor = KeyReconstructor::new();
    ...
    ///
    /// Returns error if key reconstruction fails.
    pub fn find_insertion_point(&self, key: &str) -> Result<(usize, bool), super::Error> {
        let mut reconstructor = KeyReconstructor::new();
...
impl MstNode {
    /// DAG-CBOR bytes for this node.
    ///
    /// `l` and `t` serialize as explicit nulls when absent. Upstream omitted
    /// them (`skip_serializing_if`), and since an absent field and a null field
    /// are different bytes in DAG-CBOR, that alone gave every node — down to a
    /// single-entry one — a CID no conformant implementation agrees with.
    pub fn to_bytes(&self) -> Result<Vec<u8>, Error> {
        Ok(atproto_dasl::to_vec(self)?)
    }

    /// Parse a node from DAG-CBOR bytes.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        Ok(atproto_dasl::from_slice(bytes)?)
    }

    /// CIDv1, dag-cbor, sha2-256 — this node's content address.
    pub fn cid(&self) -> Result<CidCore, Error> {
        Ok(atproto_repo::compute_cid(&self.to_bytes()?))
    }
}
