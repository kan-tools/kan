...
pub struct TreeEntry {
    ...
    ///
    /// Serialized as an explicit `null` when absent, NOT omitted — see the note
    /// on `MstNode::left` for why the distinction changes the node's CID.
    #[serde(rename = "t")]
    pub tree: Option<Cid>,
}
...
impl TreeEntry {
    ...
    pub fn reconstruct_key(&self, prev_key: &str) -> Result<String, super::Error> {
        let prefix_len = self.prefix_len as usize;

        if prefix_len > prev_key.len() {
            return Err(super::Error::InvalidPrefix {
                reason: format!(
        ...
        let suffix =
            std::str::from_utf8(&self.key_suffix).map_err(|e| super::Error::InvalidPrefix {
...
impl KeyReconstructor {
    ...
    ///
    /// Returns error if the key cannot be reconstructed.
    pub fn next(&mut self, entry: &TreeEntry) -> Result<String, super::Error> {
